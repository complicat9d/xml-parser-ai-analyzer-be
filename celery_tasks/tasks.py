...
def fetch_and_process_sales_data(self, session: AsyncSession):
    loop = asyncio.get_event_loop()
    xml_content = fetch_xml_data()
    date, products = parse_xml(xml_content)
    sale_id = loop.run_until_complete(create_sale(date, session))
    loop.run_until_complete(save_products(products, sale_id, session))

    report = generate_report(products)

    loop.run_until_complete(add_report(sale_id, report, session))
